...
class NN(Base):
    def __init__(self, safety_percentage, train_parts, train_miss, valid_parts, test_parts, test_miss,
                 train_batch_size, hidden_nodes, image_depth, num_steps,
    ...
    def reformat(self, dataset, labels, image_shape, num_labels):
        dataset = dataset.reshape((-1, image_shape[0] * image_shape[1])).astype(np.float32)
    ...
    def load_batch(self, dataset_file_paths, labels, offset, batch_size, image_shape, pixel_depth, num_labels):
        ...
        batch_data, batch_labels = self.reformat(batch_data, batch_labels, image_shape, num_labels)

        return batch_data, batch_labels, skipped_images

    @staticmethod
    def model(data, layer1_weights, layer1_biases, layer2_weights, layer2_biases, hidden_nodes):
        layer1 = tf.matmul(data, layer1_weights) + layer1_biases
        hidden = tf.nn.relu(layer1)
        if hidden_nodes > 1:
            for i in range(hidden_nodes - 1):
                hidden = tf.nn.relu(hidden)
        layer2 = tf.matmul(hidden, layer2_weights) + layer2_biases
        return layer2
    ...
    def setup(self):
        ...
        with self.graph.as_default():
            ...
            self.tf_train_dataset = tf.placeholder(
                tf.float32,
                shape=(self.train_batch_size, Base.train_image_shape[0] * Base.train_image_shape[1]))
            ...
            self.tf_valid_dataset = tf.placeholder(
                tf.float32,
                shape=(self.valid_batch_size, Base.train_image_shape[0] * Base.train_image_shape[1]))
            self.tf_test_dataset = tf.placeholder(
                tf.float32,
                shape=(self.test_batch_size, Base.test_image_shape[0] * Base.test_image_shape[1]))

            layer1_weights = tf.Variable(
                tf.truncated_normal(
                    [self.train_image_shape[0] * self.train_image_shape[1], self.total_training_classes]))
            layer1_biases = tf.Variable(tf.zeros([self.total_training_classes]))
            layer2_weights = tf.Variable(
                tf.truncated_normal(
                    [self.total_training_classes, self.total_training_classes]))
            layer2_biases = tf.Variable(tf.zeros([self.total_training_classes]))
            ...
            self.logits = NN.model(self.tf_train_dataset, layer1_weights, layer1_biases, layer2_weights, layer2_biases,
                                   self.hidden_nodes)
            ...
            self.valid_prediction = tf.nn.softmax(NN.model(self.tf_valid_dataset, layer1_weights, layer1_biases,
                                                           layer2_weights, layer2_biases, self.hidden_nodes))
            self.test_prediction = tf.nn.softmax(NN.model(self.tf_test_dataset, layer1_weights, layer1_biases,
                                                          layer2_weights, layer2_biases, self.hidden_nodes))
